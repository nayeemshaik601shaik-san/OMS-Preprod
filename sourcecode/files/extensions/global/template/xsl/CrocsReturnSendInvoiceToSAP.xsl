...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org">
    ...
    <!-- Template for transforming the root element (InvoiceDetail) -->
    <xsl:template match="/InvoiceDetail">
        <Invoice>
            <!-- Apply templates for InvoiceHeader -->
            <xsl:apply-templates select="InvoiceHeader"/>
        </Invoice>
    </xsl:template>

    <!-- Template for InvoiceHeader -->
    <xsl:template match="InvoiceHeader">
        <InvoiceDetails>
            <!-- Map InvoiceHeader attributes -->
            ...
            <xsl:attribute name="DerivedFromOrderHeaderKey">
                <xsl:value-of select="@DerivedFromOrderHeaderKey"/>
            </xsl:attribute>
            ...
            <!-- Apply templates for Order -->
            <xsl:apply-templates select="Order"/>

            <!-- Apply templates for LineDetails -->
            <xsl:apply-templates select="LineDetails"/>

            <!-- Apply templates for TotalSummary -->
            <xsl:apply-templates select="TotalSummary"/>

            <!-- Apply templates for HeaderCharges -->
            <xsl:apply-templates select="HeaderCharges"/>

            <!-- Apply templates for HeaderTaxes -->
    ...
    <xsl:template match="Order">
        <Order>
            ...
            <!-- Apply templates for PriceInfo -->
            <xsl:apply-templates select="PriceInfo"/>

            <!-- Apply templates for PersonInfoShipTo -->
            <xsl:apply-templates select="PersonInfoShipTo"/>

            <!-- Apply templates for PersonInfoBillTo -->
