...
/**
 * this class is used to process ship confirmation msg from UPS EOMS-4866
 */
public class CrocsCAShipmentPackedToShippedStatusFromUPS extends CrocsCACreateShipInputForShipmentUpdatesFromUPS
		implements CrocsConstant {
	private static final YFCLogCategory logger = YFCLogCategory
			.instance(CrocsCAShipmentPackedToShippedStatusFromUPS.class);
	...
	public Document processPackedShipmentToConfirmFromUPS(YFSEnvironment env, Document inDoc) throws Exception {
		logger.verbose(
				"Start of method processPackedShipmentToConfirmFromUPS with input: " + SCXmlUtil.getString(inDoc));
		logger.info("CrocsCAShipmentPackedToShippedStatusFromUPS : processPackedShipmentToConfirmFromUPS : inDoc: "
		...
		if (FLAG_Y.equals(cancelNonShippedQuantity))
			outDoc = performAction(env, inDoc, FLAG_Y, isCancelled);
			
		logger.verbose(
				"End of method processPackedShipmentToConfirmFromUPS with output: " + SCXmlUtil.getString(outDoc));
		return outDoc;
	}
	 ...
	 * @throws Exception
	 */
	private Document performAction(YFSEnvironment env, Document inputDocumentInitial, String actionItem,
			String isCancelled) throws Exception {
		
		logger.verbose("Start of method processShipmentConfirmUpdates with input: "
				+ SCXmlUtil.getString(inputDocumentInitial));
		...
		if (FLAG_Y.equals(actionItem)) {
			/**
			   we receive wmsCode  =5 always in case of Crocs_CA 
			   */			
			...
			logger.info("CrocsCAShipmentPackedToShippedStatusFromUPS : performAction Y : after getShipmentListForOrder"
					+ orderNo);

			// cancel order if isCancelled flag is Y
			if (FLAG_Y.equals(isCancelled)) {
				return handleCancelOrder(env, shipmentListOutput, orderReleaseListOutDoc, inputDocumentInitial);
			}
			// this will be executed for WMSCode=5
			if (!YFCCommon.isVoid(wmsCode) && "5".equals(wmsCode)) {
				shipmentListOutput = createShipmentAndReturnShipmentDetails(env, inputDocumentInitial, releaseNo,
						orderReleaseListOutDoc, shipmentListOutput);
			}
			...
			logger.info(
					"CrocsCAShipmentPackedToShippedStatusFromUPS : performAction Y : after confirmShipment" + orderNo);
		} else {
		...
		logger.verbose(
				"End of method processShipmentConfirmUpdates with output: " + SCXmlUtil.getString(inputDocumentFinal));
		return outDoc;
	}
	 ...
	 * the ChangeOrder API and for retail orders the entire release is cancelled by invoking changeRelease API.
	 * - If a shipment exists, the specific shipment is
	...
	private Document handleCancelOrder(YFSEnvironment env, Document shipmentListOutput,
        Document orderReleaseListOutDoc, Document inputDocumentInitial)throws Exception {
		//Start--EOMS-9187
		 env.setTxnObject("IsCancelled", "Y");
		//End--EOMS-9187
        if (!shipmentListOutput.getDocumentElement().hasChildNodes()) {
        	
            String strOrderHeaderKey = SCXmlUtil.getXpathAttribute(
                    orderReleaseListOutDoc.getDocumentElement(),
                    "OrderRelease/@OrderHeaderKey");
          
            //Changes as part of EOMS-10219 : START
            String orderType = SCXmlUtil.getXpathAttribute(
                    orderReleaseListOutDoc.getDocumentElement(),
                    "OrderRelease/@OrderType");

           if (A_RETAIL_ORDER_TYPE.equals(orderType)) {
				logger.verbose("Ordertype is 02::Invoking createChangeReleaseDoc::");
				
				// Calling changeRelease to modify the release.
				Document changeReleaseDoc = createChangeReleaseDoc(orderReleaseListOutDoc);
				logger.verbose("Invoking changeRelease API: " + SCXmlUtil.getString(changeReleaseDoc));

				CommonUtil.invokeAPI(env, "", API_CHANGE_RELEASE, changeReleaseDoc);
				// Changes as part of EOMS-10219 : END
			} else {
                // Calling changeOrder
                Document changeOrderInDoc = createChangeOrderDoc(strOrderHeaderKey);
                logger.verbose("changeOrderInDoc: "
            ...
            // Existing shipment cancel logic (unchanged)
            String shipmentNo = SCXmlUtil.getXpathAttribute(
                    shipmentListOutput.getDocumentElement(),
                    "Shipment/@ShipmentNo");

            String shipNode = SCXmlUtil.getXpathAttribute(
                    shipmentListOutput.getDocumentElement(),
                    "Shipment/@ShipNode");

            String sellerOrganizationCode = SCXmlUtil.getXpathAttribute(
                    shipmentListOutput.getDocumentElement(),
                    "Shipment/@SellerOrganizationCode");

            Document changeShipmentInDoc = createChangeShipmentDoc(
                    shipNode, shipmentNo, sellerOrganizationCode);

            logger.verbose("Canceling shipment ShipmentNo:" + shipmentNo);
            logger.verbose("changeShipmentInDoc: "
                 + SCXmlUtil.getString(changeShipmentInDoc));

            CommonUtil.invokeAPI(env, "", API_CHANGE_SHIPMENT, changeShipmentInDoc);
        }
	...
	private Document createChangeOrderDoc(String orderHeaderKey) {
		...
		logger.verbose("CrocsCAShipmentPackedToShippedStatusFromUPS.createChangeOrderDoc: " + SCXmlUtil.getString(doc));

		return doc;
	}

	/**
	 * Preparing changeShipment input xml to cancel order.
	 * 
	 * @param shipNode
	 * @param shipmentNo
	 * @param sellerOrganizationCode
	 * @return
	 */
	private Document createChangeShipmentDoc(String shipNode, String shipmentNo, String sellerOrganizationCode) {
		Document doc = SCXmlUtil.createDocument(E_SHIPMENT);
		Element ele = doc.getDocumentElement();
		ele.setAttribute(A_OVERRIDE_MODIFICATION_RULES, FLAG_Y);
		ele.setAttribute(A_CANCEL_REMOVED_QUANTITY, FLAG_Y);
		ele.setAttribute(A_SHIP_NODE, shipNode);
		ele.setAttribute(A_SHIPMENT_NO, shipmentNo);

		ele.setAttribute(A_ACTION, A_CANCEL);
		ele.setAttribute(A_SELLER_ORGANIZATION_CODE, sellerOrganizationCode);
		logger.verbose("CrocsCAShipmentPackedToShippedStatusFromUPS.createChangeShipmentDoc: " + SCXmlUtil.getString(doc));
		return doc;
	}
	...
	private Document invokeConfirmShipmentAPI(YFSEnvironment env, Document inputDoc) throws YFSException {
		logger.info("CrocsCAShipmentPackedToShippedStatusFromUPS: Start of method invokeConfirmShipmentAPI with input: "
	...
	private Document updateContainerDetailsForFlagY(YFSEnvironment env, Document inDoc, Document orderReleaseListOutDoc) throws Exception {
		...
		if (containersEle.hasChildNodes()) {
			...
			for (Element eachContainer : containerListEle) {
				...
				logger.verbose(
						"Calling getOrganizationList with input: " + SCXmlUtil.getString(getOrganizationListInDoc));
				...
				logger.verbose(
						"Output returned from getOrganizationList: " + SCXmlUtil.getString(getOrganizationListOutDoc));
	...
	private void updateShipmentAttributes(Document inDoc, Document shipmentInDoc) {
		...
		logger.verbose(
				"Start of method updateShipmentAttributes with shipmentInDoc: " + SCXmlUtil.getString(shipmentInDoc));
	...
	private Document getOrderReleaseList(YFSEnvironment env, Document indoc, String orderNo, String releaseNo)
		...
		// EOMS-4670 Start
		Document orderReleaseListOutDoc = CommonUtil.invokeService(env, SERVICE_GET_ORDER_RELEASE_LIST_CA,
	...
	private Document getShipmentListForOrder(YFSEnvironment env, Document inDoc, String orderNo, String releaseNo,String orderType)
		...
		if (orderReleaseEle != null) {
			...
			// EOMS-4670 Start
			Document shipmentListDoc = CommonUtil.invokeService(env, SERVICE_GET_SHIPMENT_LIST_FOR_ORDER_CA,
	...
	public Document updateSCAC(YFSEnvironment env, Document inDoc, String scac) throws Exception {
		...
		try {
			...
			inEleCC.setAttribute(CrocsXmlConstants.A_ORGANIZATION_CODE, CROCS_CA);
	...
	private Element getScacAndServiceList(YFSEnvironment env, Document inDoc, String strSCAC, String strSCACandService)
		...
		Element scacAndServiceElement = SCXmlUtil.getXpathElement(getScacAndServiceOutput.getDocumentElement(),
				"/ScacAndServiceList/ScacAndService");
	 ...
	 */
	private Document createShipmentAndReturnShipmentDetails(YFSEnvironment env, Document inDoc, String releaseNo,
			Document orderReleaseListOutDoc, Document shipmentListForOrderDoc) throws Exception {
	 ...
	 */
	private Document createNewShipment(YFSEnvironment env, String releaseNo, Document orderReleaseListOutDoc)
			throws Exception {
		...
		Document createdShipmentDoc = CommonUtil.invokeAPI(env, TEMPLATE_CREATE_SHIPMENT, API_CREATE_SHIPMENT,
				createShipmentInputDoc);

		// Wrap into <ShipmentList>
		Document shipmentListDoc = SCXmlUtil.createDocument("ShipmentList");
		shipmentListDoc.getDocumentElement()
				.appendChild(shipmentListDoc.importNode(createdShipmentDoc.getDocumentElement(), true));
	...
	private void addShipmentLineForCancellation(Document inputDocumentInitial, Document orderReleaseListOutDoc) {
		logger.verbose("Start of method addShipmentLineForCancellation with inputDocumentInitial: "
				+ SCXmlUtil.getString(inputDocumentInitial));
		logger.verbose("Start of method addShipmentLineForCancellation with orderReleaseListOutDoc: "
				+ SCXmlUtil.getString(orderReleaseListOutDoc));
		...
		for (Element remainingOrderLine : listOfOrderLine) {
			...
			shipmentLineToCancel.setAttribute(A_ITEM_ID,
					SCXmlUtil.getXpathAttribute(remainingOrderLine, STR_XPATH_ITEM_ID));
			...
			shipmentLineToCancel.setAttribute(A_UNIT_OF_MEASURE,
					SCXmlUtil.getXpathAttribute(remainingOrderLine, XPATH_ITEM_UNIT_OF_MEASURE));
		}
		logger.verbose("End of method addShipmentLineForCancellation with updated inDoc:: "
				+ SCXmlUtil.getString(inputDocumentInitial));
	}
	...
	private void updateTrackingNo(YFSEnvironment env, String orderNo, Document orderReleaseListOutDoc,
		...
		logger.info("CrocsCAShipmentPackedToShippedStatusFromUPS: updateTrackingNo: start: " + orderNo);
		logger.verbose("CrocsCAShipmentPackedToShippedStatusFromUPS: updateTrackingNo: orderReleaseListOutDoc: "
				+ XMLUtil.getXMLString(orderReleaseListOutDoc));
		...
		try {
			logger.verbose("CrocsCAShipmentPackedToShippedStatusFromUPS: updateTrackingNo: input to changeOrder: "
					+ XMLUtil.getXMLString(changeOrderInDoc));
	...
	/**
     * Prepares input XML for changeRelease API to cancel/modify a release.
     *
     *
     *
     * Changes as part of EOMS-10219
     *
     */
    private Document createChangeReleaseDoc(Document orderReleaseListOutDoc) throws Exception {

		logger.verbose("Executing CrocsCAShipmentPackedToShippedStatusFromUPS.createChangeReleaseDoc ::: START ");
		logger.verbose("Input for executing createChangeReleaseDoc method ::::  "
				+ SCXmlUtil.getString(orderReleaseListOutDoc));

		Element eleOrderReleaseList = orderReleaseListOutDoc.getDocumentElement();
		Element orderReleaseEle = SCXmlUtil.getChildElement(eleOrderReleaseList, E_ORDER_RELEASE);

		if (orderReleaseEle == null) {
			throw new YFSException("OrderRelease element not found in OrderReleaseList");
		}

		Document doc = SCXmlUtil.createDocument("OrderRelease");
		Element rootEle = doc.getDocumentElement();

		rootEle.setAttribute(A_ACTION, "CANCEL");
		rootEle.setAttribute(A_ORDER_HEADER_KEY, orderReleaseEle.getAttribute(A_ORDER_HEADER_KEY));
		rootEle.setAttribute(A_ORDER_RELEASE_KEY, orderReleaseEle.getAttribute(A_ORDER_RELEASE_KEY));
		rootEle.setAttribute(A_ENTERPRISE_CODE, orderReleaseEle.getAttribute(A_ENTERPRISE_CODE));

		rootEle.setAttribute(A_REASON_CODE, "UPS_CANCEL");
		rootEle.setAttribute(A_REASON_TEXT, "UPS requested cancellation of entire release");
		rootEle.setAttribute(A_SELECT_METHOD, "WAIT");
		rootEle.setAttribute(A_OVERRIDE, "Y");

		logger.verbose("End of method createChangeReleaseDoc with output: " + SCXmlUtil.getString(doc));
