...
public class CrocsShipmentConfirmationNotification implements CrocsConstant {
	private static final YFCLogCategory logger = YFCLogCategory.instance(CrocsShipmentConfirmationNotification.class);
	...
	public Document confirmShipmentNotification(YFSEnvironment env, Document inDoc) throws Exception {
        ...
        /* EOMS-13313 this method is used to remove the Lines from OrderList which is not part of Shipment*/
        Document filteredOrderListDoc = getFilteredOrderListDoc(env, getOrderListOutDoc, inDocEle);
		Document outDoc = prepareDocForSFCC(env,inDoc, filteredOrderListDoc);
    ...
    /** EOMS-13313 this method is used to remove the Lines from OrderList which is not part of Shipment and quantity is 0.
     * @param env
     * @param getOrderListOutDoc
     * @param shipmentEle
     * @return
     */
    public static Document getFilteredOrderListDoc(YFSEnvironment env, Document getOrderListOutDoc, Element shipmentEle) {
        logger.verbose("start of method confirmShipmentNotification.getFilteredOrderListDoc :OrderListindoc: " + SCXmlUtil.getString(getOrderListOutDoc));
        logger.verbose("start of method confirmShipmentNotification.getFilteredOrderListDoc : shipmentEle: " + XMLUtil.getElementXMLString(shipmentEle));

        Set<String> shippedItemIds = new HashSet<>();
        Element shipmentLinesEle = SCXmlUtil.getChildElement(shipmentEle,E_SHIPMENT_LINES);
        NodeList shipmentLineEle = shipmentLinesEle.getElementsByTagName(E_SHIPMENT_LINE);
        for(int i=0; i<shipmentLineEle.getLength();i++){
            Element shipmentLine = (Element)shipmentLineEle.item(i);
            double quantity = Double.parseDouble(shipmentLine.getAttribute(V_QUANTITY));
            if(quantity>0){
                shippedItemIds.add(shipmentLine.getAttribute(A_ITEM_ID));
            }
        }

        Element orderListEle = getOrderListOutDoc.getDocumentElement();
        NodeList orderLines = orderListEle.getElementsByTagName(E_ORDER_LINE);

        for (int i = orderLines.getLength() - 1; i >= 0; i--) {
            Element orderLine = (Element) orderLines.item(i);
            Element itemEle = SCXmlUtil.getChildElement(orderLine ,E_ITEM);
            String itemId = itemEle.getAttribute(A_ITEM_ID);
            if (!shippedItemIds.contains(itemId)) {
                orderLine.getParentNode().removeChild(orderLine);
            }
        }
        logger.verbose("End of method confirmShipmentNotification.getFilteredOrderListDoc with OrderListindoc: " + SCXmlUtil.getString(getOrderListOutDoc));
	...
	public Document prepareDocForSFCC(YFSEnvironment env, Document inDoc, Document getOrderListDoc) throws Exception {
		...
		String enteredBy = SCXmlUtil.getAttribute(orderEle, A_ENTERED_BY);
		String customerPONo = SCXmlUtil.getAttribute(orderEle, CustomerPONo);
		if (!YFCCommon.isVoid(enteredBy) && !YFCCommon.isVoid(CustomerPONo)) {
			orderEle.setAttribute(A_ENTERED_BY, orderDateParsed.format(dateFormatter));
			orderEle.setAttribute(CustomerPONo, customerPONo);
		}
		...
		if (!YFCCommon.isVoid(currency)) {
			switch (currency.toUpperCase()) {
				...
				case VAL_CA_CURRENCY: // "CAD"
					...
					break;
				case VAL_AU_CURRENCY: // "AUD"
					currencySymbol = VAL_AU_CURRENCY_SYMBOL;
					break;
                case VAL_GB_CURRENCY: // "GBP"
                    currencySymbol = VAL_GB_CURRENCY_SYMBOL;
                    break;
                case VAL_EU_CURRENCY: // "EUR"
                    currencySymbol = VAL_EU_CURRENCY_SYMBOL;
                    break;
                case VAL_SG_CURRENCY: // "SGD" //EOMS-11285
					currencySymbol = VAL_SG_CURRENCY_SYMBOL;
					break;
                case VAL_KR_CURRENCY: // "KRW" //EOMS-11550
					currencySymbol = VAL_KR_CURRENCY_SYMBOL;
					break;
				default:
					currencySymbol = VAL_US_CURRENCY_SYMBOL;
					break;
	...
	public void getUniquePromotionId(Document getOrderListDoc) {
		...
		try {
			...
			for (Element orderLine : orderLineList) {
				...
				// EOMS - 7995 Sum up discount amounts from lineOverallTotals
