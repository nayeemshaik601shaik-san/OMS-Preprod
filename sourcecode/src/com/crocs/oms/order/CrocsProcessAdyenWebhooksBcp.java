 ...
 */
public class CrocsProcessAdyenWebhooksBcp implements CrocsConstant {
	private static YFCLogCategory logger = YFCLogCategory.instance(CrocsProcessAdyenWebhooksBcp.class);
	 ...
	 * @throws Exception
	 */
	public static Document cancelOrderOnAuthFailure(String strOrderHeaderKey, Document docChangeOrderInput) throws YFSException {
	 ...
	 * @throws YFSException
	 */
	public static Document cancelOrderOnForterFailure(Document docChangeOrderInput) throws YFSException {
	...
	/**
	 * Description: This below method helps to append the payment Details tag when Webhook response success.
	 *
	 * @param env
		...
		try {
			if (docGetOrderListOutput != null
				...
				if (V_SUCCESS.equalsIgnoreCase(strForterMessage)) {
					...
					docChangeOrderInput = cancelOrderOnForterFailure(docChangeOrderInput);
				}
			}
	...
	public static void updateAuthorizationIdForPostPayments(YFSEnvironment env, Element eleCapture,
		...
		try {
			...
			if (A_TRUE_STRING.equalsIgnoreCase(strSuccess)) {
				...
				docChangeOrderInput = cancelOrderOnAuthFailure(strOrderHeaderKey, docChangeOrderInput);
			}
