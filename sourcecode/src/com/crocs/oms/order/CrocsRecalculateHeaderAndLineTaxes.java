...
public class CrocsRecalculateHeaderAndLineTaxes {
	...
	public Document crocsRecalculateHeaderAndLineTaxes(YFSEnvironment env, Document getOrderDetails) {
		...
		try {
			...
			processHeaderTaxes(getOrderDetails, taxLineItemList, strEnterpriseCode);

			processLineTaxes(getOrderDetails, taxLineItemList, strEnterpriseCode);
	 ...
	 */
	private Document processHeaderTaxes(Document getOrderDetails, YFCIterable<YFCElement> taxLineItemList,
		...
		if (CrocsConstant.CROCS_US.equalsIgnoreCase(strEnterpriseCode)) {
			processUSTax(vertexShippingLineItemList, getOrderDetails);
		} else if (CrocsConstant.CROCS_CA.equalsIgnoreCase(strEnterpriseCode)) {
			processCATax(vertexShippingLineItemList, getOrderDetails);
		}
		return getOrderDetails;
	}
	 ...
	 * @return
	 */
	private Document processCATax(List<YFCElement> vertexShippingLineItemList, Document getOrderDetails) {
		...
		for (YFCElement lineItem : vertexShippingLineItemList) {
			...
			lineItem.getChildren(CrocsConstant.V_TAXES).forEach(tax -> {
				...
				Element eleHeaderTax = createHeaderTaxElement(docUpdatedHeaderTaxes, tax, strVProductName,
						nlHeaderTaxes);
				if (eleHeaderTax != null) {
					eleNewHeaderTaxes.appendChild(eleHeaderTax);
					logger.verbose("CrocsRecalculateHeaderAndLineTaxes : processCATax - HeaderTax Element: "
							+ SCXmlUtil.getString(eleHeaderTax));
				}
			});
		}
	 ...
	 * @return
	 */
	private Element createHeaderTaxElement(Document doc, YFCElement tax, String productName, Element existingTaxes) {
	 ...
	 * @return
	 */
	private Document processLineTaxes(Document getOrderDetails, YFCIterable<YFCElement> taxLineItemList,
			String strEnterpriseCode) {
		...
		for (int i = 0; i < nleleOrderLine.getLength(); i++) {
			...
			if (!YFCCommon.isVoid(taxLineItem)) {
				if (CrocsConstant.CROCS_US.equalsIgnoreCase(strEnterpriseCode)) {
					...
					processLineCATax(taxLineItem, eleOrderLine);
	 ...
	 */
	private void processLineCATax(YFCElement lineTaxes, Element eleOrderLine) {
