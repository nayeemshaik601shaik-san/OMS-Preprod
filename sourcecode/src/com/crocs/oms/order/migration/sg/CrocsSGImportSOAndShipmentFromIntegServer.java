...
public class CrocsSGImportSOAndShipmentFromIntegServer implements CrocsConstant {
	private static YFCLogCategory logger = YFCLogCategory.instance(CrocsSGImportSOAndShipmentFromIntegServer.class.getName());
	...
	<Shipment EnteredBy="Migration" ActualDeliveryDate="" ActualShipmentDate="2025-04-05T08:15:47.000+0000" BillToCustomerId="" BuyerOrganizationCode="" CarrierServiceCode="AUSPOST" Currency="USD" DeliveryTS="" DocumentType="0001" EnterpriseCode="CROCS_AU" ExpectedDeliveryDate="" ExpectedShipmentDate="2025-04-05T08:15:47.000+0000" History="N" OrderNo="SGLOC000015" ShipDate="2025-04-05" ShipNode="3011" ShipToCustomerId="" PipelineKey="202504031215582621376" ReceivingNode="" ReleaseNo="" SellerOrganizationCode="CROCS_AU" ShipmentKey="" ShipmentNo="AULOC000015_1" Status="1400" TrackingNo="1Z6F857YYW260869975|STP">
	...
	public Document importSOAndShipment(YFSEnvironment env, Document importSOAndShipmentInDoc) {
		...
		try {
			...
			if (CrocsConstant.STR_STATUS_CANCELLED.equals(
				...
				Document crocsCreateSOMigrationSyncServOutDoc = CommonUtil.invokeService(env,"CrocsSGCreateSOMigrationSyncServ", importSalesOrderYdoc.getDocument());
				...
				for (YFCNode node : nodes) {
					...
					if (E_SHIPMENT.equals(node.getNodeName())) {
						...
						if (E_ORDER.equals(importSalesOrderYdocEle.getTagName())) {
							...
							Document crocsCreateSGSOMigrationSyncServOutDoc = CommonUtil.invokeService(env,
									"CrocsSGCreateSOMigrationSyncServ", importSalesOrderYdoc.getDocument());
							
							logger.info("Migartion : OMS_UPDATE : Dropped Sales Order message to Queue : " + XMLUtil.getXMLString(crocsCreateSGSOMigrationSyncServOutDoc));
							
							if (importSalesOrderYdocEle.getAttribute(CrocsConstant.OrderNo)
									.equals(crocsCreateSGSOMigrationSyncServOutDoc.getDocumentElement()
				...
				if (isEligibleForShipmentProcessing) {
					...
					Document crocsPostSOShipMessageSyncServerOutDoc = CommonUtil.invokeService(env,
							"CrocsSGPostSOShipMessageSyncServer",
		...
		logger.info("Migration : OMS_UPDATE : CrocsSGImportSOAndShipmentFromIntegServer.importSOAndShipment: END");
