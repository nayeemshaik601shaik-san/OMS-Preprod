...
public class CrocsSGMigarationUtil implements CrocsConstant {
	private static final YFCLogCategory logger = YFCLogCategory.instance(CrocsSGMigarationUtil.class);
	...
	public static boolean validateMandatoryAttribute(YFCElement importOrderYdocEle,
       ...
       case CrocsConstant.A_SO_MIGRATION_CROCS_SG_PIPELINE_PROPERTY:
			...
			for (YFCElement OrderLine : OrderLines) {
				...
				String SOMigrationPipelineKey = YFSSystem.getProperty(CrocsConstant.A_SO_MIGRATION_CROCS_SG_PIPELINE_PROPERTY);
				if (!YFCCommon.isVoid(SOMigrationPipelineKey)) {
					...
					errorDescription = "Provided value for " + CrocsConstant.A_PIPELINE_KEY + " is "
							...
							+ CrocsConstant.A_SO_MIGRATION_CROCS_SG_PIPELINE_PROPERTY + " at xpath: " + path;
					break;
				}
			}
				break;
				
			case A_RO_MIGRATION_CROCS_SG_PIPELINE_PROPERTY:

				OrderLines = importOrderYdocEle.getElementsByTagName(E_ORDER_LINE);
				for (YFCElement OrderLine : OrderLines) {

					String ROMigrationPipelineKey = YFSSystem.getProperty(A_RO_MIGRATION_CROCS_SG_PIPELINE_PROPERTY);
					if (!YFCCommon.isVoid(ROMigrationPipelineKey)) {
						...
						errorDescription = "Provided value for " + A_PIPELINE_KEY + " is "
								...
								+ ", Expected is valid pipeline provided in SMA with property name "
								+ A_RO_MIGRATION_CROCS_SG_PIPELINE_PROPERTY + " at xpath: " + path;
						break;
					}
				}
				break;
			
		case CrocsConstant.A_SO_SHIPMENT_MIGRATION_CROCS_SG_PIPELINE_PROPERTY:
			 ...
			 **/
				...
				String shipmentOMigrationAUPipelineKey = YFSSystem.getProperty(CrocsConstant.A_SO_SHIPMENT_MIGRATION_CROCS_SG_PIPELINE_PROPERTY);
				if (!YFCCommon.isVoid(shipmentOMigrationAUPipelineKey)) {
					...
					errorDescription = "Provided value for " + CrocsConstant.A_PIPELINE_KEY + " is "
							...
							+ ", Expected is valid pipeline provided in SMA with property name "
							+ CrocsConstant.A_SO_SHIPMENT_MIGRATION_CROCS_SG_PIPELINE_PROPERTY + " at xpath: " + path;
					break;
				}
