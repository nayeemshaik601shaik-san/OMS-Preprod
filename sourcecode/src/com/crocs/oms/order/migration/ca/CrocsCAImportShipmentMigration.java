...
public class CrocsCAImportShipmentMigration implements CrocsConstant{
	private static YFCLogCategory logger = YFCLogCategory.instance(CrocsCAImportShipmentMigration.class.getName());
	 ...
	 * <p> Hover over method for more details <p>
      ...
      <pre>{@code
		...
		<Shipment EnteredBy="ImportOrder" ActualDeliveryDate="" ActualShipmentDate="2025-01-22T08:15:47.000+0000" BillToCustomerId="" 
		BuyerOrganizationCode="" CarrierServiceCode="dayton-economy2" Currency="USD" DeliveryTS="" DocumentType="0001" 
		EnterpriseCode="CROCS_US" ExpectedDeliveryDate="" ExpectedShipmentDate="2025-01-22T08:15:47.000+0000" History="N" 
		OrderNo="61866081CUS" ShipDate="2025-01-22" ShipNode="1005" ShipToCustomerId="" PipelineKey="2025032412404198521" 
		ReceivingNode="" ReleaseNo="" SellerOrganizationCode="CROCS_US" ShipmentKey="" ShipmentNo="63307081_1" Status="1400" 
		TrackingNo="1Z6F857YYW26086997|UPS">
			<FromAddress AddressLine1="484 NW 53rd Ave" AddressLine2="" City="Miami" Country="US" EMailID="nannasolano@gmail.com" 
			FirstName="Nazareth" LastName="Solano" MobilePhone="(178) 626-2445" State="FL" ZipCode="33126-5040"/>
			<ToAddress AddressLine1="5050 Factory Shops Blvd" AddressLine2="Suite 505" City="Castle Rock" Country="US" 
			EMailID="Hemant.Kumar23@ibm.com" FirstName="RAJ123" LastName="ALEX123" OtherPhone="(347) 893-4334" PersonID="" 
			State="CO" ZipCode="80108"/>
			<ShipmentLines>
			<ShipmentLine ItemID="10013833" ItemDesc="Denim Bow" OrderHeaderKey="20250325112638102203" 
			OrderLineKey="20250325112638102205" OrderNo="61866081CUS" ShipmentLineNo="1" Quantity="2" UnitOfMeasure="EACH"/>
			<ShipmentLine ItemID="210581-6WY-M5W7" ItemDesc="Classic Mary Jane Clog" OrderHeaderKey="20250325112638102203" 
			OrderLineKey="20250325112638102205" OrderNo="61866081CUS" ShipmentLineNo="1" Quantity="1" UnitOfMeasure="EACH"/>
			</ShipmentLines>
		</Shipment>
     * }
	...
	public Document importShipment(YFSEnvironment env, Document importShipmentInDoc) throws Exception {
		...
		try {
			...
			String importShipmentMandatoryAttr[] = { CrocsConstant.A_DOCUMENT_TYPE,
					...
					CrocsConstant.A_SO_SHIPMENT_MIGRATION_CA_PIPELINE_PROPERTY };
			...
			if (isValidForImportOrderProcesingFlag) {
				...
				CrocsCAMigarationUtil.updateCarrierServiceCode(env,importShipmentYdocEle,importShipmentYdocEle.getAttribute(CrocsConstant.EnterpriseCode));
	...
	private void importSOProcesingReadiness(YFSEnvironment env, YFCElement importShipmentYdocEle) throws Exception {
		try {
			...
			if (!CrocsCAMigarationUtil.isValidSalesOrder(getOrderListOutput)) {
	...
	private boolean isValidForImportShipmentProcesing(YFCElement importShipmentYdocEle,
			...
			for (String ImportOrderMandatoryAttrtribute : importShipmentMandatoryAttr) {
				...
				if (CrocsCAMigarationUtil.validateMandatoryAttribute(importShipmentYdocEle,
