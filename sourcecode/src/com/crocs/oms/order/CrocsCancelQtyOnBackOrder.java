...
public class CrocsCancelQtyOnBackOrder implements CrocsXmlConstants {

	private static YFCLogCategory logger = YFCLogCategory.instance(CrocsCancelQtyOnBackOrder.class);
	 ...
	 * @throws Exception
	 */
	public Document prepareInputForCancelQtyOnBackOrder(YFSEnvironment env, Document inDoc) throws YFSException {
			...
			for (int i = 0; i < orderLineList.getLength(); i++) {
				...
				if (!YFCCommon.isVoid(eleBackOrderedFrom)) {
					...
					if (CrocsConstant.VAL_STATUS_1300.equalsIgnoreCase(strBackOrderedStatus)
						...
						// EOMS- 10220  Start	
						String noteText = CrocsConstant.ORDER_LINE_CANCEL_NOTE_TEXT.replace(CrocsConstant.QTY,strBackOrderedQty).replace(CrocsConstant.CANCELLED_REASON, cancelReason);
						docChangeOrderInput = crocsOrderCancellationNotesUtil.getNotesTag(eleChangeOrderLine, docChangeOrderInput, noteText,cancelReason);
						// EOMS- 10220  End	
