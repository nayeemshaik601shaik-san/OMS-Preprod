 ...
 * 
 */
public class CrocsFraudCheckOnRetry {

	private static YFCLogCategory logger = YFCLogCategory.instance(CrocsFraudCheckOnRetry.class);
	
	public static Document validateFraudCheckOnRetryStatus(YFSEnvironment env, Document inDoc) throws YFSException {
		
		logger.verbose("CrocsFraudCheckOnRetry : validateFraudCheckOnRetryStatus: START" + XMLUtil.getXMLString(inDoc));
		...
		try {
			...
			logger.verbose("CrocsFraudCheckOnRetry.validateFraudCheckOnRetryStatus :Expection" + e.getMessage());
			
		}
		logger.verbose("CrocsFraudCheckOnRetry : validateFraudCheckOnRetryStatus:: END:: " );
