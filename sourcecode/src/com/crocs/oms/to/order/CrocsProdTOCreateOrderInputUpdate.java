 ...
 */
public class CrocsProdTOCreateOrderInputUpdate implements CrocsXmlConstants{

	private static final YFCLogCategory logger = YFCLogCategory.instance(CrocsProdTOCreateOrderInputUpdate.class);
	...
	public Document  updateOrderAttributes(YFSEnvironment env, Document inDoc) {
		...
		try {
			...
			// Apply transformations only for Standard Transfer Orders
	...
	private void updateReqDeliveryDate(YFSEnvironment env, Element eleOrder) throws Exception {
		...
		if (CrocsConstant.A_RUSH_TO.equalsIgnoreCase(orderType)) {
			...
			int daysToAdd = calculateDaysToAdd(ccOut);
			// Add offset days
	...
	private void updatePersonInfoBillTo(YFSEnvironment env, Document inDoc, Element eleOrder)
		...
		Document orgOut = api.executeFlow(env, CrocsConstant.A_CROCS_GET_ORGANIZATION_LIST, orgDoc.getDocument());
		...
		// Replace PersonInfoBillTo with CorporatePersonInfo from Organization
		Element corpPersonInfo = (Element) eleOrg.getElementsByTagName(CrocsXmlConstants.E_CORPORATE_PERSONINFO)
				.item(0);
		if (!YFCObject.isNull(corpPersonInfo)) {
			Element newPIB = (Element) inDoc.importNode(corpPersonInfo, true);
			// Setting Orgname as First name to send it to WMS
			String strOrgName = eleOrg.getAttribute(CrocsXmlConstants.A_ORGANIZATION_NAME);
	...
	private void fetchAndUpdateOrderItems(YFSEnvironment env, Element eleOrder) throws Exception {
	    ...
	    enrichOrderLines(orderLines, upcToItemMap);
	}
	 ...
	 */
	private void enrichOrderLines(NodeList orderLines, Map<String, Element> upcToItemMap) {
	    for (int i = 0; i < orderLines.getLength(); i++) {
	        Element eleOL = (Element) orderLines.item(i);
