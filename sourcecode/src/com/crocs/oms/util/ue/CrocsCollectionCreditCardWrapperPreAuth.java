...
import com.crocs.oms.payment.util.PaymentUtilsPreAuth;
 ...
 * 
 */
public class CrocsCollectionCreditCardWrapperPreAuth extends CrocsProcessAdyenWebhooks implements CrocsConstant, YFSCollectionCreditCardUE {


	private static YFCLogCategory logger = YFCLogCategory.instance(CrocsCollectionCreditCardWrapperPreAuth.class);
	...
	public YFSExtnPaymentCollectionOutputStruct collectionCreditCard(YFSEnvironment env,YFSExtnPaymentCollectionInputStruct inStruct) throws YFSUserExitException {
		...
		if (!CrocsMigarationUtil.isImportOrderEligibleForPayment(env, inStruct.orderHeaderKey)) {
			...
			/** ChargeType = AUTHORIZATION would be in only case when there is a need to
			 * either modify the auth amount  or  extend the card expiration  before capture  hence AMOUNTUPDATES call to adyen is needed*/
			//EOMS-643: Delta Auth Changes for Credit Card : START
			strAdyenRequest = CrocsConstant.A_EVENT_CODE_AMOUNT_UPDATES;
			Document getPaymentListOutDoc = PaymentUtilsPreAuth.getPaymentList(env, inStruct.paymentKey);
			String strTotalAuthorizationAmt = SCXmlUtil.getXpathAttribute(getPaymentListOutDoc.getDocumentElement(), XPATH_PAYMENT_LIST_TOTAL_AUTHORIZED);
			double dTotalAuthorizationAmt = Double.parseDouble(strTotalAuthorizationAmt);
			double dRequestAmt = Math.abs(inStruct.requestAmount);
			dOrderTotal = dTotalAuthorizationAmt + dRequestAmt;
			dOrderTotal = dOrderTotal * Math.pow(10, 2);
			iValue = (int) Math.round(dOrderTotal);
			
			logger.info("amount Updates Response: " + inStruct.orderNo + ": Authorization Amount"+ dOrderTotal);
			
			//EOMS-643: Delta Auth Changes for Givex : END
		}
		...
		try {
			...
			if (CrocsConstant.A_EVENT_CODE_AMOUNT_UPDATES.equalsIgnoreCase(strAdyenRequest)) {
				jsonOrderObj.put(CrocsConstant.A_INDUSTRY_USAGE, CrocsConstant.A_DELAYED_CHARGE);
			}
		...
		try {
			...
			if (status != null && status.equalsIgnoreCase(CrocsConstant.ADYEN_STATUS_RECEIVED)) {
				...
				if (CrocsConstant.A_EVENT_CODE_AMOUNT_UPDATES.equalsIgnoreCase(strAdyenRequest)) {
					...
					outStruct.authorizationExpirationDate = PaymentUtilsPreAuth.getAuthorizationExpirationDate(env,inStruct.creditCardType.toUpperCase(),inStruct.enterpriseCode);
					logger.info("amount Updates Response: " + inStruct.orderNo + ": Authorization Amount"+ inStruct.requestAmount);
					logger.info("amount Updates Response: authorizationExpirationDate "+ outStruct.authorizationExpirationDate);
